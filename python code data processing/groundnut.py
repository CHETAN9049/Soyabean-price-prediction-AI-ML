...
def process_file(file_path):
    try:
        ...
        logger.warning(f"⚠️ Skipping {os.path.basename(file_path)}: Cannot read file - {e}")
        return None

    title_tag = soup.find('span', id='cphBody_Label3')
    if not title_tag or not title_tag.text.strip():
        logger.warning(f"⚠️ Skipping {os.path.basename(file_path)}: Title not found")
        return None

    title = title_tag.text.strip()
    month, year = extract_month_year(title)
    if not month or not year:
        logger.warning(f"⚠️ Skipping {os.path.basename(file_path)}: Month/Year not found")
        return None

    table = soup.find('table', id='cphBody_DataGrid_PriMon')
    if not table:
        logger.warning(f"⚠️ Skipping {os.path.basename(file_path)}: Table not found")
        return None
    ...
    if not data:
        logger.warning(f"⚠️ Skipping {os.path.basename(file_path)}: No valid data rows found")
        return None
...
def select_dropdown_and_submit(page, year, month, context):
    try:
        logger.info(f"Processing Groundnut Month={month} Year={year}")
        ...
        page.select_option(commodity_selector, value="10")  # Groundnut
        time.sleep(0.5)
        ...
        file_name = f"groundnut_{year}_{month}.html"
...
def main():
    ...
    if all_data:
        ...
        output_file = "Groundnut_Prices_Normalized.xlsx"
